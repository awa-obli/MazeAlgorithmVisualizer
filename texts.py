...
特点：生成的迷宫分支丰富，结构自然，与Prim类似。"""
        ...
        "recursive": {
            "title": "4. 递归分割",
            "content": """核心思想：将空间用十字分成四个子空间，在三面墙上挖洞，递归分割直到空间不足。
...
特点：生成速度快，生成的迷宫结构规整，通常比较简单。"""
    ...
    "pathfinding": {
        ...
        "dfs": {
            "title": "1. 深度优先搜索 (DFS)",
            "content": """核心思想：维护一个栈，从起点开始不断前进（入栈），走不通就回溯（出栈），尝试其他方向。
...
特点：实现简单，但找到的不一定是最短路径。"""
        ...
        "bfs": {
            "title": "2. 广度优先搜索 (BFS)",
            "content": """核心思想：维护一个队列，从起点开始逐层向外扩散搜索（将所有邻居格点入队）。
...
特点：保证找到最短路径，但内存占用较大。"""
        ...
        "dijkstra": {
            "title": "3. Dijkstra算法",
            "content": """核心思想：始终选择当前距离起点最近的格子进行探索，并更新其相邻格子的距离。

流程：
1. 初始化优先队列，加入起点
2. 优先队列不为空：
   - 从优先队列中取出距离起点最近的格点作为当前格点
   - 若当前格点为终点 → 回溯路径并返回
...
注意：由于二维网格迷宫中所有边权重相同，在这里dijkstra寻路和bfs寻路无异！"""
        ...
        "gbfs": {
            "title": "4. 贪心最佳优先搜索 (GBFS)",
            "content": """核心思想：只考虑启发式距离，总是选择当前看起来离目标最近的格子进行探索。

流程：
1. 初始化优先队列，加入起点
2. 优先队列不为空：
   - 从优先队列中取出距离终点最近的格点作为当前格点
   - 若当前格点为终点 → 回溯路径并返回
   - 遍历四个方向：若相邻格点在范围内、为通路且未访问 →
       计算该格点到终点的曼哈顿距离作为优先级，加入队列
3. 优先队列空 → 寻路失败

特点：方向性强，能快速找到目标，但不保证最短路径。"""
        },
        "astar": {
            "title": "5. A*算法",
            "content": """核心思想：估计总代价（分为当前代价和预估代价），优先探索总代价最小的方向。结合了Dijkstra的系统性和GBFS的方向性。
...
特点：效率高，路径最优，适合大多数场景下的路径搜索。"""
        ...
        "d-dfs": {
            "title": "6. 双向深度优先 (D-DFS)",
            "content": """核心思想：从起点和终点同时进行深度优先搜索，当两个方向相遇时找到路径。

流程：
1. 初始化两个栈和访问记录，分别从起点和终点开始
2. 交替扩展两个方向的搜索：
   - 获取当前方向的栈顶格点
   - 遍历四个方向：若相邻格点在迷宫范围内、为通路且未在当前方向访问过 →
       - 入栈、记录已访问
       - 若该格点已被另一方向访问过 → 路径相遇
   - 若无可访问方向 → 出栈（回溯）
3. 构建完整路径（起点→相遇点→终点）

特点：在某些情况下比单向dfs更高效，找到的不一定是最短路径。"""
        },
        "d-bfs": {
            "title": "7. 双向广度优先 (D-BFS)",
            "content": """核心思想：从起点和终点同时进行广度优先搜索，当两个方向相遇时找到路径。

流程：
1. 初始化两个队列和访问记录，分别从起点和终点开始
2. 交替扩展两个方向的每一层节点：
   - 取出当前方向的队首格点
   - 遍历四个方向：若相邻格点在迷宫范围内、为通路且未在当前方向访问过 →
       - 入队、记录已访问
