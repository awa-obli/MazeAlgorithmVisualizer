...
特点：生成速度快，生成的迷宫结构规整，通常比较简单。"""
    ...
    "pathfinding": {
        ...
        "dfs": {
            "title": "1. 深度优先搜索 (DFS)",
            "content": """思路：从起点开始沿一个方向走到底，走不通就回溯，尝试另一方向，直到找到终点。
...
特点：实现简单，但找到的不一定是最短路径"""
        ...
        "bfs": {
            "title": "2. 广度优先搜索 (BFS)",
            "content": """思路：从起点开始逐层向外扩散搜索，保证找到的路径是最短路径（步数最少）。
...
特点：保证最短路径，但内存占用较大。"""
        ...
        "astar": {
            "title": "3. A*算法",
            "content": """思路：结合曼哈顿距离评估节点优先级，优先探索代价最小（最可能接近终点）的方向。
